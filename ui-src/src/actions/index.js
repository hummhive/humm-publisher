...
export function newPostDispatch(title, author, pubdate, lastupdate, tags, content, status) {
  return dispatch => fetchPOST('/fn/Posts/CreatePost', {title, tags, content, status})
    .then(response => {
      dispatch(addNewPost(response.hash, response.uuid, title, author, pubdate, lastupdate, tags, content, status));
    }).catch(err => {
...
export function deletePostDispatch(hash, status) {
  return dispatch => fetchPOST('/fn/Posts/DeletePost', {hash, status})
    .then(response => {
...
export function editPostDispatch(prevHash, uuid, title, author, pubdate, lastupdate, tags, content, status) {
  return dispatch => fetchPOST('/fn/Posts/EditPost', {hash: prevHash, uuid, title, tags, content, status})
    .then(response => {
      dispatch(editPost(response.hash, uuid, title, author, pubdate, lastupdate, tags, content, status, prevHash));
    }).catch(err => {
